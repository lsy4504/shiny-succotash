...
<form action="<%=request.getContextPath() %>/login/loginCheck.jsp" method="post">
	<ul>
		<li>
			...
			아이디 : <input type="text" name="mem_id" value="<%=Objects.toString(idChecked,"") %>" />
			<label>
			<input type="checkbox" name="idChecked" value="idSaved" <%=StringUtils.isNotBlank(idChecked)?"checked":"" %>/>아이디 저장하기
			</label>
			<%-- 체크하면전송 체크안하면 비전송 
